...
class BaseCarrierInstructions(BaseModel):
...
class BaseValueAddingServiceRequest(BaseModel):
...
class BaseCarrierServiceRequest(BaseModel):
    ...
    value_adding_services: Optional[list[BaseValueAddingServiceRequest]] = Field(None, description="Value adding services to use in conjunction with the main carrier service, i.e. for `home_delivery` this might be value-adds such as `cash_on_delivery`, `verification_signature`, `insurance` etc.")

class BaseCarrierRequestModel(BaseModel):
    id: str = Field(..., description="Glueys ID that identifies the carrier in our system, e.g. `poste_italiane`, `yodel`.")
    ...
    instructions: Optional[BaseCarrierInstructions] = Field(None, description="Will be forwarded to carrier if they support it. Any special instructions for the carrier, e.g. `Leave at front door`, `Blue side up`.")
