...
from app.api.v1.common.models.base_models import Document, MetaData, PaperlessLabelType, paperless_type_descriptions
...
class ParcelResponseModel(BaseParcelResponseModel):
    ...
    barcodes: Optional[Barcode] = Field(None, description="For label services. The barcodes that are visible on the parcel.")
    labels_codes: Optional[LabelCodesResponseModel] = Field(None, description="For label services. The labels, qr codes and / or parcel locker pin codes that was generated by Gluey / the carrier.")
    label_meta_data: Optional[list[MetaData]] = Field(None, description="If available. Meta data related to the label printed.")

class ShipmentLabel(LabelResponseBaseModel):
    type: PaperlessLabelType = Field(..., description=f"The type of paperless label that was generated by Gluey / the carrier. It can be one of the following:\n{get_enum_description(PaperlessLabelType, paperless_type_descriptions)}")
    pass

class ShipmentLabelResponse(BaseModel):
    qr_code_barcode: ShipmentLabel = Field(..., description="The qr code / barcode that was generated by Gluey / the carrier.")
    barcodes: Barcode = Field(..., description="The qr code / barcodes value that was generated by Gluey / the carrier.")
    label_meta_data: Optional[list[MetaData]] = Field(None, description="If available. Meta data related to the paperless label printed.")

class ShipmentResponseModel(BaseShipmentResponseModel):
    paperless_label: Optional[ShipmentLabelResponse] = Field(None, description="If making a request for a paperless service. The qr codes / barcodes for the paperless service.")
